...
static void activate(GtkApplication *app, gpointer userdata)
{
  ...
  if (!alreadyRunning)
  {
    ...
    wallpaperMgrWnd =
        (GtkWidget *)gtk_builder_get_object(builder, "WallpaperManagerWindow");
    monitorWnd = (GtkWidget *)gtk_builder_get_object(builder, "MonitorWindow");
    appSettingsWnd =
        (GtkWidget *)gtk_builder_get_object(builder, "SettingsWindow");
    monitorListBox = (GtkWidget *)gtk_builder_get_object(
        builder, "MainWindow_MonitorListBox"
    );
    versionLabel =
        (GtkWidget *)gtk_builder_get_object(builder, "MainWindow_VersionLabel");
    wallpaperListBox = (GtkWidget *)gtk_builder_get_object(
        builder, "WallpaperManagerWindow_WallpaperListBox"
    );
    wallpaperComboBox = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_WallpaperComboBox"
    );
    xPosSpinBtn = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_XPosSpinBtn"
    );
    yPosSpinBtn = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_YPosSpinBtn"
    );
    widthSpinBtn = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_WidthSpinBtn"
    );
    heightSpinBtn = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_HeightSpinBtn"
    );
    monitorNameLabel = (GtkWidget *)gtk_builder_get_object(
        builder, "MonitorWindow_MonitorNameLabel"
    );
    targetFpsComboBox = (GtkWidget *)gtk_builder_get_object(
        builder, "SettingsWindow_TargetFpsComboBox"
    );
    ...
    gtk_window_set_application(
        GTK_WINDOW(wallpaperMgrWnd), GTK_APPLICATION(app)
    );
    gtk_window_set_application(GTK_WINDOW(monitorWnd), GTK_APPLICATION(app));
    gtk_window_set_application(
        GTK_WINDOW(appSettingsWnd), GTK_APPLICATION(app)
    );
